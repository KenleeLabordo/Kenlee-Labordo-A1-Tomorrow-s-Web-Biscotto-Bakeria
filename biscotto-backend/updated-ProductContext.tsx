...
export const ProductProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const fetchData = async () => {
      try {
        ...
        const productsData = productsResponse.products.map((p: any) => ({
          id: parseInt(p._id, 36) % 1000000, // Convert MongoDB _id to number for compatibility
          name: p.name,
          price: p.price,
          category: p.category,
          stock: p.stock,
          ...
          _id: p._id // Keep original MongoDB ID for updates/deletes
        }));
  ...
  const updateHomeSettings = async (newSettings: Partial<HomeSettings>) => {
    try {
      ...
      await settingsAPI.updateHome(updatedSettings);
  ...
  const updateAboutSettings = async (newSettings: Partial<AboutSettings>) => {
    try {
      ...
      await settingsAPI.updateAbout(updatedSettings);
